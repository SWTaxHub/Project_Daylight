...
print('BlackBoard data Shape:')

print(blackboard_data.shape)

blackboard_data_condensed = blackboard_data['Staff_ID'].isin(matchingEMPLIDS['MatchingValues'])
...
blackboard_data_reducedv1 = blackboard_data_reduced[['Course_ID', 'Grade_Centre_Title', 'Weight_of_Column']]
...
reduced_mean_weights_df = blackboard_data_reducedv1.groupby(['Course_ID', 'Grade_Centre_Title'], as_index=False)['Weight_of_Column'].mean()
...
reduced_mean_weights_df['Grade_Centre_Title'] = reduced_mean_weights_df['Grade_Centre_Title'].str.lower()

# Count how frequently each unique 'AssessmentComponentName' occurs in the DataFrame
Reduced_Grade_CentreTitle_counts = reduced_mean_weights_df['Grade_Centre_Title'].value_counts()

Grade_CentreTitle_counts = mean_weights_df['Grade_Centre_Title'].value_counts()

# Display the counts of unique 'AssessmentComponentName' values
print('Assessment Counts from BB:')
print(Reduced_Grade_CentreTitle_counts)

# Save the counts to a CSV file
Grade_CentreTitle_counts.to_csv('GradeCentreTitle_Frequency.csv', header=True)
Reduced_Grade_CentreTitle_counts.to_csv('ReducedGradeCentreTitle_Frequency.csv', header=True)
...
BB_course_grade_counts = blackboard_data_reduced.groupby('Course_ID')['Grade_Centre_ID'].nunique().reset_index()
...
matching_rows = mean_weights_df[mean_weights_df['Course_ID'].isin(referenceValues)]
