...
print('timesheet_cas_OT_daily_weekly prior to adjusted_daily hours')
print(timesheet_cas_OT_daily_weekly.columns)
'''
#Older version of Step 4 code commented out as part of Query DQ11
# Step 4: First reduce the total_hours by overtime hours and then apply cumsum()
...
#Revised code for Step 4 Daily code DQ11

# Apply the conditional logic: If PIN_NM equals 0 then adjusted_daily_hours = total_hours otherwise calculate as before
timesheet_cas_OT_daily_weekly['adjusted_daily_hours'] = np.where(
    timesheet_cas_OT_daily_weekly['PIN_NM'] == 'OT',
    0,
    timesheet_cas_OT_daily_weekly['total_hours'] - 
    timesheet_cas_OT_daily_weekly['cal_ot_span_weekend_hours'] - 
    timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] - 
...
#print('First run on timesheet_cas_OT_daily_weekly')
#print(timesheet_cas_OT_daily_weekly.columns)
...
'''
#Older version of Step 4 code commented out as part of Query DQ11
# Step 4: First reduce total_hours by overtime hours and daily OT hours, then apply cumsum() for weekly thresholds
...
#Revised code for Step 4 Weekly Calc
# Apply the conditional logic: If PIN_NM equals 0 then adjusted_daily_hours = total_hours otherwise calculate as before
timesheet_cas_OT_daily_weekly['adjusted_weekly_hours'] = np.where(
    timesheet_cas_OT_daily_weekly['PIN_NM'] == 'OT',
    0,
    timesheet_cas_OT_daily_weekly['total_hours'] - 
    timesheet_cas_OT_daily_weekly['cal_ot_span_weekend_hours'] - 
...
print('2nd run on timesheet_cas_OT_daily_weekly')
print(timesheet_cas_OT_daily_weekly.columns)
...
# # Step 7: Calculate cal_OT_hours based on the provided rules
# # Define the conditions
# conditions = [
#     # 1. if incremental_weekly_ot_hours == total_hours
#     timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] == timesheet_cas_OT_daily_weekly['total_hours'],
#
#     # 2. if cal_ot_span_weekend_hours == total_hours
#     timesheet_cas_OT_daily_weekly['cal_ot_span_weekend_hours'] == timesheet_cas_OT_daily_weekly['total_hours'],
#
#     # 3. if cal_ot_span_as_hours = 0 and cal_ot_span_bs_hours = 0 and incremental_daily_ot_hours = 0 and incremental_weekly_ot_hours != 0
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] != 0),
#
#     # 4. if cal_ot_span_as_hours = 0 and cal_ot_span_bs_hours = 0 and incremental_daily_ot_hours != 0 and incremental_weekly_ot_hours = 0
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] != 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] == 0),
#
#     # 5. if cal_ot_span_as_hours = 0 and cal_ot_span_bs_hours != 0 and incremental_daily_ot_hours = 0 and incremental_weekly_ot_hours = 0
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] != 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] == 0),
#
#     # 6. if cal_ot_span_as_hours != 0 and cal_ot_span_bs_hours = 0 and incremental_daily_ot_hours = 0 and incremental_weekly_ot_hours = 0
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] != 0) &
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] == 0),
#
#     # 7. if cal_ot_span_as_hours != 0 and cal_ot_span_bs_hours != 0 and incremental_daily_ot_hours = 0 and incremental_weekly_ot_hours = 0
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] != 0) &
#     (timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] != 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] == 0) &
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] == 0),
#
#     # 8. if (incremental_weekly_ot_hours + cal_ot_span_bs_hours) >= total_hours
#     (timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] + timesheet_cas_OT_daily_weekly[
#         'cal_ot_span_bs_hours']) >= timesheet_cas_OT_daily_weekly['total_hours'],
#
#     # 9. if (incremental_daily_ot_hours + cal_ot_span_bs_hours) >= total_hours
#     (timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] + timesheet_cas_OT_daily_weekly[
#         'cal_ot_span_bs_hours']) >= timesheet_cas_OT_daily_weekly['total_hours']
# ]
#
# # Define the corresponding values for each condition
# choices = [
#     timesheet_cas_OT_daily_weekly['total_hours'],  # Rule 1
#     timesheet_cas_OT_daily_weekly['total_hours'],  # Rule 2
#     timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'],  # Rule 3
#     timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'],  # Rule 4
#     timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'],  # Rule 5
#     timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'],  # Rule 6
#     timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours'] + timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'],
#     # Rule 7
#     timesheet_cas_OT_daily_weekly['total_hours'],  # Rule 8
#     timesheet_cas_OT_daily_weekly['total_hours']  # Rule 9
# ]
#
# # Rule 9: Fallback to the max value between the three combinations
# # Combination 1: cal_ot_span_bs_hours + cal_ot_span_as_hours
# # Combination 2: incremental_daily_ot_hours + cal_ot_span_as_hours
# # Combination 3: incremental_weekly_ot_hours + cal_ot_span_as_hours
# combination_1 = timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours'] + timesheet_cas_OT_daily_weekly['cal_ot_span_as_hours']
# combination_2 = timesheet_cas_OT_daily_weekly['incremental_daily_ot_hours'] + timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours']
# combination_3 = timesheet_cas_OT_daily_weekly['incremental_weekly_ot_hours'] + timesheet_cas_OT_daily_weekly['cal_ot_span_bs_hours']
#
# # Find the maximum value across the three combinations
# fallback_choice = pd.DataFrame({
#     'combination_1': combination_1,
#     'combination_2': combination_2,
#     'combination_3': combination_3
# }).max(axis=1)
#
# # Apply the conditions and assign to cal_OT_hours
# timesheet_cas_OT_daily_weekly['cal_OT_hours'] = np.select(conditions, choices, default=fallback_choice)
...
timesheet_cas_OT_daily_weekly['cal_shift_top_up'] = np.select(
    [
        # Condition 1: DATE WORKED >= Rule - greater than date and penalty_reptm == 'no base paid'
        (timesheet_cas_OT_daily_weekly['DATE WORKED'] >= timesheet_cas_OT_daily_weekly['Rule - greater than date']) &
        (timesheet_cas_OT_daily_weekly['penalty_reptm'] == 'no base paid'),

        # Condition 2: DATE WORKED < Rule - greater than date, penalty_reptm == 'no base paid', and PIN_NM == 'SHIFT15'
        (timesheet_cas_OT_daily_weekly['DATE WORKED'] < timesheet_cas_OT_daily_weekly['Rule - greater than date']) &
        (timesheet_cas_OT_daily_weekly['penalty_reptm'] == 'no base paid') &
        (timesheet_cas_OT_daily_weekly['PIN_NM'] == 'SHIFT15'),

        # Condition 3: DATE WORKED < Rule - greater than date, penalty_reptm == 'no base paid', and PIN_NM == 'SHIFT50'
        (timesheet_cas_OT_daily_weekly['DATE WORKED'] < timesheet_cas_OT_daily_weekly['Rule - greater than date']) &
        (timesheet_cas_OT_daily_weekly['penalty_reptm'] == 'no base paid') &
        (timesheet_cas_OT_daily_weekly['PIN_NM'] == 'SHIFT50')
    ],
    [
        # Calculation for each condition
        timesheet_cas_OT_daily_weekly['base_rate'] * timesheet_cas_OT_daily_weekly['total_hours'],  # Condition 1
        timesheet_cas_OT_daily_weekly['base_rate'] * timesheet_cas_OT_daily_weekly['total_hours'] * 0.85,  # Condition 2
        timesheet_cas_OT_daily_weekly['base_rate'] * timesheet_cas_OT_daily_weekly['total_hours'] * 0.5  # Condition 3
    ],
    default=0  # Default value if no conditions are met
)
